...
Thanks to neuroscience development, designers are now able to limit the interface to a single complex chip directly implemented in the brain of the user. Such technology is mainly dedicated to disabled people who can't use the here-above interfaces. By example, Brown university researchers succeed in helping paralysed woman to move a robotic arm thanks to a wireless chip implemented in her brain \cite{brownRobotic}. In a short future, the idea to transmit information directly from the brain of the user to a machine could become more affordable and become a new standard in term of interface design.
...
\label{sct:experiment}
    ...
    During our experiments, people tends to choose the skeuomorph calculator for the first case, The travelling bill. Indeed, we suppose that due to simplicity of operations, people think the simpler calculator will be enough to solve every asked questions. A little problem arose, when we ask them to give us only the details for money spent on taxi fares due to the fact that the computation steps weren't labelled. In general, results provided by users were correct. Another point noticed by user was the fact that the skeuomorph calculator was not working as the ones that they daily use (especially for the percent function). Such point can be very bad due to the fact that the usability provided by the skeuomorph design is no more available. When the flat calculator was used for the first exercise, users tends to use as many features as possible (Usage of labels, usage of dedicated functions). As a result, the users were able to tell us about the details of the bill. \\
    ...
    In general, users couldn't use the full potential of the flat calculator due to the lack of information. At first sight, and due to a limited designed, users appeared to be lost and limited their actions to the simplest one (addition, subtraction, division, multiplication) instead of exploiting the dedicated function to compute percentage or solve basic equation. Several users admitted that they wanted to use the flat calculator for everyday operations (shopping list, bills,\ldots) if they could follow a small training session because it can counterbalance a lack of mathematical knowledge.\\

    The last question of our poll was about the new design of iOS 7 proposed by \textit{Apple}.
    Users estimated that the new flat design is more beautiful and abstract without much loss in terms of usability. Older users admitted that the a flat design required a necessary but fast adaptation:
